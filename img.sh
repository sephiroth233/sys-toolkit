...
_img_is_sourced=0
_IMG_SCRIPT_PATH="${BASH_SOURCE[0]:-$0}"
...
if [[ -n "$_IMG_SCRIPT_PATH" && "$_IMG_SCRIPT_PATH" != /* ]]; then
  _IMG_SCRIPT_PATH="$(cd -P "$(dirname "$_IMG_SCRIPT_PATH")" 2>/dev/null && pwd)/$(basename "$_IMG_SCRIPT_PATH")"
fi
...
_harden_aws_files() {
  [[ -d "${HOME}/.aws" ]] && chmod 700 "${HOME}/.aws" 2>/dev/null || true
  [[ -f "${HOME}/.aws/credentials" ]] && chmod 600 "${HOME}/.aws/credentials" 2>/dev/null || true
  [[ -f "${HOME}/.aws/config" ]] && chmod 600 "${HOME}/.aws/config" 2>/dev/null || true
}
...
_load_config() {
  ...
  source "$CONFIG_FILE"
  if [[ -z "${PROFILE:-}" || -z "${BUCKET:-}" || -z "${ENDPOINT:-}" || -z "${PUBLIC_BASE_URL:-}" ]]; then
    echo "❌ 配置文件不完整，请重新运行: img setup" >&2
    return 1
  fi
}
...
_img_setup() {
  ...
  if [[ -z "$profile" || -z "$access_key" || -z "$secret_key" || -z "$endpoint" || -z "$bucket" || -z "$public_url" ]]; then
    echo "❌ 配置信息不能为空，请重新运行: img setup" >&2
    return 1
  fi
  ...
  echo "    access key:   $(_mask_secret "$access_key")"
  echo "    secret key:   $(_mask_secret "$secret_key")"
  echo
  ...
  mkdir -p "$CONFIG_DIR"
  {
    echo "# img-r2 config — generated by img setup"
    printf 'PROFILE=%s\n' "$(_shell_quote "$profile")"
    printf 'BUCKET=%s\n' "$(_shell_quote "$bucket")"
    printf 'ENDPOINT=%s\n' "$(_shell_quote "$endpoint")"
    printf 'PUBLIC_BASE_URL=%s\n' "$(_shell_quote "$public_url")"
  } > "$CONFIG_FILE"
  ...
  local url
  if url="$(_img_upload "$test_file" 2>&1)"; then
    echo "  ✅ 上传成功: ${url}"
    rm -f "$test_file"
...
_img_upload() {
  ...
  for file in "$@"; do
    ...
    aws --profile "$PROFILE" \
      ...
      s3 cp "$file" "s3://${BUCKET}/${key}" >/dev/null || return 1
...
_img_delete() {
  ...
  for arg in "$@"; do
    local key
    if [[ "$arg" == http* ]]; then
      if [[ "$arg" != "${PUBLIC_BASE_URL}/"* ]]; then
        echo "❌ URL 不属于当前公开域名: ${PUBLIC_BASE_URL}" >&2
        return 1
      fi
    ...
    if [[ -z "$key" || "$key" == "$arg" && "$arg" == http* ]]; then
      echo "❌ 无法解析删除目标: ${arg}" >&2
      return 1
    fi
  ...
  if command -v aws &>/dev/null; then
    if _confirm "  卸载 AWS CLI（brew uninstall awscli）？"; then
      if command -v brew &>/dev/null; then
        ...
        echo "  ⚠️  未找到 Homebrew，请按你的安装方式手动卸载 AWS CLI"
      fi
    else
      echo "  跳过。"
    fi
